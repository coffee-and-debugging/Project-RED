  ...
  const handleBloodTestSubmit = async (bloodTestData) => {
    try {
      if (isEditMode) {
        ...
        await hospitalApi.put(`/hospital-dashboard/assignments/${currentAssignment}/update_blood_test/`, bloodTestData);
        setSuccess('Blood test results updated successfully!');
      } else {
        // Submit new blood test
        await hospitalApi.post(`/hospital-dashboard/assignments/${currentAssignment}/submit_blood_test/`, bloodTestData);
      ...
      setError('Failed to submit blood test: ' + (error.response?.data?.detail || error.message));
    }
  };
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            ...
            <TableBody>
              {donors.map((donor) => (
                <TableRow key={donor.assignment_id}>
                  ...
                  <TableCell>
                    <Chip label={donor.blood_group} color="primary" variant="outlined" />
                  </TableCell>
                  ...
                  <TableCell>
                    <Chip 
                      ...
                      color={
                        donor.assignment_status === 'completed' ? 'success' : 
                        donor.assignment_status === 'scheduled' ? 'info' : 'warning'
                      } 
                    />
                  </TableCell>
                  <TableCell>
      ...
      <Dialog open={predictionDialogOpen} onClose={() => setPredictionDialogOpen(false)} maxWidth="md" fullWidth>
        <DialogTitle>
          Health Prediction for {selectedDonor?.first_name} {selectedDonor?.last_name}
        </DialogTitle>
        <DialogContent>
            ...
            <Box sx={{ mt: 2 }}>
              ...
              <Box sx={{ mb: 2 }}>
                <Typography variant="body2">
                  <strong>Sugar Level:</strong> {selectedDonor.blood_test.sugar_level} mg/dL
                </Typography>
                <Typography variant="body2">
                  <strong>Hemoglobin:</strong> {selectedDonor.blood_test.hemoglobin} g/dL
                </Typography>
                <Typography variant="body2">
                  <strong>Uric Acid:</strong> {selectedDonor.blood_test.uric_acid_level} mg/dL
                </Typography>
                <Typography variant="body2">
                  <strong>WBC Count:</strong> {selectedDonor.blood_test.wbc_count} cells/mcL
                </Typography>
                <Typography variant="body2">
                  <strong>RBC Count:</strong> {selectedDonor.blood_test.rbc_count} million cells/mcL
                </Typography>
                <Typography variant="body2">
                  <strong>Platelet Count:</strong> {selectedDonor.blood_test.platelet_count} platelets/mcL
                </Typography>
              </Box>
              ...
              {selectedDonor.blood_test.prediction_confidence && (
                <Typography variant="body2" color="textSecondary">
                  <strong>Confidence Level:</strong> {selectedDonor.blood_test.prediction_confidence}%
                </Typography>
              )}
            </Box>
          ) : (
            <Typography>No health prediction available for this donor.</Typography>
          )}
        </DialogContent>
        <DialogActions>
